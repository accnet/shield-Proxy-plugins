...
class ShieldSettings {
    ...
    public function shield_settings_page_callback() {
        ...
        <div class="shield-dashboard">
            ...
            <div class="shield-grid">
                    ...
                    <div class="shield-card">
                        ...
                        <div class="shield-card-body">
                            <?php if ($conn_status === 'connected' && !empty($conn_data['shieldId'])) : ?>
                                <div style="display:flex;gap:12px;background:#f0fdf4;border:1px solid #bbf7d0;padding:12px 16px;border-radius:8px;margin-bottom:20px;font-size:12px;">
                                    <div>⚡</div>
                                    <div>
                                        <span style="color:#166534;font-weight:700;">CONNECTED TO SHIELD SYSTEM</span>
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-top:24px;border-top:1px solid #f1f5f9;padding-top:20px;">
                    ...
                    <div class="shield-card">
                        ...
                        <div class="shield-card-body" style="padding:0">
                            <?php if (empty($v2_credentials)) : ?>
                                <div style="padding:32px;text-align:center;color:#64748b;font-size:12px;">Chưa có WooCommerce store manager (site2.local) nào được bootstrap kết nối.</div>
                            <?php else : ?>
                ...
                <!-- Right Column (Credentials Inspector & Webhook logs) -->
                <div>
            ...
            <div class="shield-grid" style="grid-template-columns: 1fr; margin-top:24px;">
                    ...
                    <div class="shield-card">
                        ...
                        <div class="shield-card-body" style="padding:0">
                            <?php if (empty($latest_payments)) : ?>
                                <div style="padding:24px;text-align:center;color:#64748b;font-size:12px;">Chưa nhận được giao dịch nào qua webhook.</div>
                            <?php else : ?>
                                <div class="shield-table-wrap">
                                    <table class="shield-table">
                                        <thead>
                    ...
                    <div class="shield-card">
                        ...
                        <div class="shield-card-body" style="padding:0">
                            <?php if (empty($latest_events)) : ?>
                                <div style="padding:24px;text-align:center;color:#64748b;font-size:12px;">Chưa nhận được event nào từ Stripe.</div>
                            <?php else : ?>
                                <div class="shield-table-wrap">
                                    <table class="shield-table">
                                        <thead>
    ...
    public function render_stripe_webhook_panel($mode, $entry) {
        ...
        foreach ($fields as $label => $info) {
            ...
            $html .= '      <span style="display:block;font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;margin-bottom:4px;">' . esc_html($label) . '</span>';
            $html .= '      <div style="display:flex;gap:6px;align-items:center;">';
            
            if ($info['copy']) {
                $html .= '        <input type="text" value="' . esc_attr($val) . '" style="flex:1;height:28px;font-size:11px;font-family:monospace;border:1px solid #cbd5e1;border-radius:4px;background:#f8fafc;padding:0 8px;" readonly>';
                $html .= '        <button type="button" class="button button-small" onclick="navigator.clipboard.writeText(\'' . esc_js($val) . '\'); alert(\'Copied!\');" style="height:28px;line-height:26px;">Copy</button>';
            } else {
                $style = $is_error ? 'color:#ef4444;background:#fef2f2;border:1px solid #fecaca;padding:4px 8px;border-radius:4px;font-size:11px;' : 'font-size:11px;color:#334155;word-break:break-all;font-family:monospace;';
                $html .= '        <span style="' . $style . '">' . esc_html($val) . '</span>';
            }
            $html .= '      </div>';
            $html .= '    </div>';
        }
