...
paypal.Buttons({
    createOrder: function (data, actions) {
        return customInterval().then(function () {
            var purchaseUnits = JSON.parse(JSON.stringify(window.wooCheckoutFormInfo.purchase_units))
            var applicationContext = {
                brand_name: 'merchant',
                user_action: 'CONTINUE',
            }
            applicationContext.shipping_preference = window.wooCheckoutFormInfo.shipping_preference
    ...
    onApprove: function (data, actions) {
        parent.postMessage({
            name: 'wootify-paypalApprovedOrder-custom',
            value: {
                order_id: data.orderID,
            }
        }, '*')
        parent.postMessage('wootify-paypalCloseCreditForm-custom', '*')
        parent.postMessage('wootify-paypalMakeIframeCreditFormNormal-custom', '*')
    ...
    onError: function (err) {
        ...
        parent.postMessage({
            name: 'wootify-paypalOpenCreditFormError-custom',
...
function listenerPaypal(event) {
    if ((typeof event.data === 'object') && event.data.name === 'wootify-paypalSendOrderInfo-custom') {
...
setInterval(function () {
    if (window.oldHeightPaymentForm != document.getElementById('paypal-button-container').offsetHeight) {
        ...
        parent.postMessage({
            name: 'wootify-paypalBodyResizeCreditForm-custom',
...
setInterval(function () {
    if (document.querySelector('[id*="paypal-overlay-uid"]')) {
        parent.postMessage('wootify-paypalMakeFullIframeCreditForm-custom', '*')
...
function customInterval() {
    return new Promise(function (resolve, reject) {
        setInterval(function () {
            if (window.wooCheckoutFormInfo.purchase_units) {
                resolve(window.wooCheckoutFormInfo.purchase_units);
            }
        }, 100)
    })
};