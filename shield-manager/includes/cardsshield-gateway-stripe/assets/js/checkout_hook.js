jQuery(function ($) {
    ...
    function getStripeLinkExpectedOrigin() {
        var proxyUrl = $('#WOOTIFY_stripe_link_current_proxy_url').data('value')
            || $('#endpoint_stripe_link_current_proxy_url').data('value');
    ...
    function handleStripeLinkConfirmationToken(payload) {
        ...
        if (payload.shipping_address) {
            ...
            if (sName) {
                var sNames2 = sName.trim().split(/\s+/).filter(Boolean);
