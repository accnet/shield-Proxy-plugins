jQuery(document).ready(function ($) {
    ...
    WOOTIFY_checkout_form.on('checkout_place_order', function () {
        if ($('input[name="payment_method"]:checked').val() === 'WOOTIFY_paypal') {
            ...
            setTimeout(function () {
                if (!window.WOOTIFY_paypal_checkout_error) {
                    $('.blockUI').hide();
                    $('#cs-pp-loader').show();
                    setTimeout((function () {
                        $('#cs-pp-loader').hide();
    ...
    $(document).on('checkout_error', function () {
        if ($('input[name="payment_method"]:checked').val() == 'WOOTIFY_paypal') {
            ...
            window.WOOTIFY_paypal_checkout_error = true;
        }
    })
    ...
    function handleShowHidePaypalButton() {
        ...
        if (currentMethod === 'endpoint_paypal') {
            return;
        }
        if (currentMethod == 'WOOTIFY_paypal' && $('#wootify-paypal-button-setting').data('value') === OPT_CS_PAYPAL_SETTING_CHECKOUT) {
    ...
    function listenerPaypal(event) {
        if (event.data === "wootify-paypalRequestFromBlacklist") {
            setInterval(function () {
                ...
                $('.wc_payment_method.payment_method_WOOTIFY_paypal').hide();
            }, 100)
        }
