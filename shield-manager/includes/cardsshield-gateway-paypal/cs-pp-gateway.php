...
if ( ! defined( 'ABSPATH' ) || class_exists( 'WC_WOOTIFY_Gateway' ) || ! class_exists( 'WC_Payment_Gateway' ) ) {
    return;
}
require_once(plugin_dir_path(__FILE__) . 'utils.php');
class WC_WOOTIFY_Gateway extends WC_Payment_Gateway {
    ...
    public static $WOOTIFY_paypal_is_inited = false;
    ...
    public function __construct() {
        $this->id                 = 'WOOTIFY_paypal'; // payment gateway plugin ID
        ...
        if(!self::$WOOTIFY_paypal_is_inited) {
            self::$WOOTIFY_paypal_is_inited = true;
...
*/',
    ...
    private function WOOTIFY_render_checkout_credit_form() {
        ...
        if (!$nextProxyUrl || $this->get_option('paypal_button') !== OPT_CS_PAYPAL_SETTING_CHECKOUT) {
            return;
        }
...
//        }
    ...
    public function process_payment( $order_id ) {
        ...
        if ($isEnableEndpointMode) {
            $shieldUrl = WC()->session->get('wootify-paypal-proxy-active-url');
            $activatedProxy = $shieldUrl ? ['id' => null, 'url' => $shieldUrl] : null;
        } else {
            $activeProxyId = WC()->session->get('wootify-paypal-proxy-active-id');
            $activatedProxy = findActivatedProxyDataById(get_option(OPT_WOOTIFY_PAYPAL_PROXIES, []), $activeProxyId);
        }
...
//            }
    ...
    public function add_WOOTIFY_shield_url_value( $column, $wc_order_id ) {
        ...
        if ($wc_order->get_payment_method() === 'WOOTIFY_paypal') {            
